...
def train(args, model, pair_data_loader, blip_data_loader, c4_data_loader, optimizer, epoch_info, device, scheduler, config, scalar, tokenizer):
    model.train()  
    ...
    accelerator_clip_grad_norm = config['accelerator'].get('CLIP_GRAD_NORM', None)
    ...
    for i, ((image, visual_token_image, org_texts), (image2, visual_token_image2, org_texts2), c4_samples) in enumerate(metric_logger.log_every(zip(pair_data_loader, blip_data_loader, c4_data_loader), print_freq, header, step_per_epoch, epoch_info)):
        ...
        if scalar is not None:
            ...
            if global_step % accelerator_gradient_accumulate_steps == 0:
                if accelerator_clip_grad_norm is not None:
                    scalar.unscale_(optimizer)
                    torch.nn.utils.clip_grad_norm_(model.parameters(), accelerator_clip_grad_norm)
...
def main(args, config):
    ...
    blip_data_loader = torch.utils.data.DataLoader(blip_dataset, batch_size=config['batch_size'],
                                               ...
                                               pin_memory=True,
                                               drop_last=False,
                                               collate_fn=blip_dataset.collate_fn
                                              )
    ...
    train(args, model, pair_data_loader, blip_data_loader, c4_data_loader, optimizer, epoch_info, device, lr_scheduler, config,
          scalar, tokenizer)
    dist.barrier()
